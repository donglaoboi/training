...
(a=this.navMenuList.length-1);a=this.navMenuList[a];if(a.extraPathList&&a.extraPathList.length){var b=a.path+"/"+a.extraPathList.join("/"),c=a.pathWithParams.indexOf("?");0<c&&(b+=a.pathWithParams.slice(c));return b}return a.pathWithParams||a.path},getLinkPath:function(a){this.appRootPath&&this.appRootPath.length&&0!==a.indexOf(this.appRootPath)&&(a=this.appRootPath+a);a=a.split("/");var b=this.appRootPath.split("/").length;a[b]=this.linkBasePath.split("/").slice(-1);return a=a.join("/")}},watch:{navMenuList:function(a){if(0<
...
"denied"===b&&moqui.notifyMessages("Browser notifications disabled, if you want them use browser notification settings to allow")})}});window.addEventListener("popstate",function(){moqui.webrootVue.setUrl(window.location.pathname+window.location.search)});
