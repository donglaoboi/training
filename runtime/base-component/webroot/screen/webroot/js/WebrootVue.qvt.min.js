...
moqui.loadComponent=function(a,b,c){var d=moqui.urlExtensions.js,e=moqui.urlExtensions.vue,f=moqui.urlExtensions.vuet;if("string"===typeof a){var g=a.indexOf("?");if(0<g){var h=a.slice(0,g);var n=a.slice(g+1)}else h=a}else{h=a.path;var m=a.extraPath;n=a.search;var l=a.bodyParameters}console.log("Load Component "+JSON.stringify(a)+" Window Width "+window.innerWidth+" Quasar Platform: "+JSON.stringify(Quasar.Platform.is)+" search: "+n);!(600>=window.innerWidth||Quasar.Platform.is.mobile)||n&&-1!==n.indexOf("_uiType")||
(n=(n||"")+"\x26_uiType\x3dmobile");if(g=moqui.componentCache.get(h))console.info("found cached component for path "+h+": "+JSON.stringify(g)),b(g);else{var p=h;a.renderModes&&0<=a.renderModes.indexOf(e)&&(p+="."+e);if(p.slice(-e.length)===e)return console.info("loadComponent vue "+p+(c?" id "+c:"")),a={type:"GET",url:p,error:moqui.handleLoadError,success:function(k,t,r){205===r.status?(k=r.getResponseHeader("X-Redirect-To"),moqui.webrootVue.setUrl(k)):(k||b(moqui.NotFound),t=0<=r.getResponseHeader("Cache-Control").indexOf("max-age"),
moqui.isString(k)&&0<k.length?(k=httpVueLoader.parse(k,p.substr(0,p.lastIndexOf("/")+1)),t&&moqui.componentCache.put(h,k),b(k)):b(moqui.NotFound))}},l&&!$.isEmptyObject(l)&&(a.type="POST",a.data=l),$.ajax(a);var q=h.slice(-d.length)===d;!q&&a.renderModes&&0<=a.renderModes.indexOf(d)&&(p+="."+d,q=!0);q||(p+="."+f);m&&0<m.length&&(p+="/"+m);n&&0<n.length&&(p+="?"+n);console.info("loadComponent "+p+(c?" id "+c:""));a={type:"GET",url:p,error:moqui.handleLoadError,success:function(k,t,r){if(205===r.status)k=
...
e.prop("disabled",!0),setTimeout(function(){e.prop("disabled",!1)},3E3));var f=Object.keys(this.fields).length?new FormData:new FormData(this.$refs.qForm.$el);$.each(this.fields,function(l,p){moqui.isArray(p)?p.forEach(function(q){f.append(l,q)}):f.set(l,p||"")});b=[];for(var g=f.entries()[Symbol.iterator]();;){var h=g.next();if(h.done)break;var n=h.value;h=n[0];n=n[1];moqui.isString(n)&&n.startsWith("__")&&f.set(h,"");!this.excludeEmptyFields||n&&n.length||b.push(h)}for(g=0;g<b.length;g++)f["delete"](b[g]);
f.set("moquiSessionToken",this.$root.moquiSessionToken);c&&f.set(c,d);this.parentCheckboxSet&&this.parentCheckboxSet.addCheckboxParameters&&this.parentCheckboxSet.addCheckboxParameters(f);this.$root.loading++;var m=new XMLHttpRequest;m.open(this.method,this.$root.appRootPath+this.action,!0);m.responseType="blob";m.withCredentials=!0;m.onload=function(){if(200===this.status){a.$root.loading--;var l=m.getResponseHeader("Content-Disposition");if(!l||-1===l.indexOf("attachment")&&-1===l.indexOf("inline")){var p=
new FileReader;p.onload=function(r){r=r.target.result;try{a.handleResponse(JSON.parse(r))}catch(v){a.handleResponse(r)}};p.readAsText(this.response)}else{var q=this.response;p="";l&&-1!==l.indexOf("attachment")&&(l=/filename[^;=\n]*=((['"]).*?\2|[^;\n]*)/.exec(l),null!=l&&l[1]&&(p=l[1].replace(/['"]/g,"")));if("undefined"!==typeof window.navigator.msSaveBlob)window.navigator.msSaveBlob(q,p);else{var k=window.URL||window.webkitURL,t=k.createObjectURL(q);p?(q=document.createElement("a"),"undefined"===
typeof q.download?window.location.href=t:(q.href=t,q.download=p,document.body.appendChild(q),q.click())):window.location.href=t;setTimeout(function(){k.revokeObjectURL(t)},100)}}}else moqui.handleLoadError(this,this.statusText,"")};m.setRequestHeader("Accept","application/json");m.send(f)},handleResponse:function(a){var b=!1;a&&moqui.isPlainObject(a)?(b=moqui.notifyMessages(a.messageInfos,a.errors),a.screenUrl&&0<a.screenUrl.length?this.$root.setUrl(a.screenUrl):a.redirectUrl&&0<a.redirectUrl.length&&
(window.location.href=a.redirectUrl)):console.warn("m-form no response or non-JSON response: "+JSON.stringify(a));(a=this.submitHideId)&&0<a.length&&$("#"+a).modal("hide");(a=this.submitReloadId)&&0<a.length&&this.$root.reloadContainer(a);(a=this.submitMessage)&&a.length?(b=eval('"'+a+'"'),moqui.webrootVue.$q.notify($.extend({},moqui.notifyOpts,{message:b})),moqui.webrootVue.addNotify(b,"success")):b||moqui.webrootVue.$q.notify($.extend({},moqui.notifyOpts,{message:"Submit successful"}))}},mounted:function(){var a=
this,b=$(this.$el);this.focusField&&this.focusField.length&&b.find('[name^\x3d"'+this.focusField+'"]').addClass("default-focus").focus();b.find('button[type\x3d"submit"], input[type\x3d"submit"], input[type\x3d"image"]').on("click",function(){a.buttonClicked=this})}});
...
q.startsWith("__")||(moqui.isArray(q)?q.forEach(function(k){d.append(p,k)}):d.set(p,q))});for(var e=[],f=[],g="",h=null,n=d.entries()[Symbol.iterator]();;){var m=n.next();if(m.done)break;var l=m.value;m=l[0];l=l[1];0===l.trim().length||"moquiSessionToken"===m||"moquiFormName"===m||0<m.indexOf("[]")||(0<m.indexOf("_op")||0<m.indexOf("_not")||0<m.indexOf("_ic")?e.push({name:m,value:l}):(f.push(m),this.bodyParameterNames&&0<=this.bodyParameterNames.indexOf(m)?(h||(h={}),h[m]=l):(0<g.length&&(g+="\x26"),
g+=encodeURIComponent(m)+"\x3d"+encodeURIComponent(l))))}for(n=0;n<e.length;n++)m=e[n],l=m.name.substring(0,m.name.indexOf("_")),0<=f.indexOf(l)&&(0<g.length&&(g+="\x26"),g+=encodeURIComponent(m.name)+"\x3d"+encodeURIComponent(m.value));a&&b&&b.trim().length&&(0<g.length&&(g+="\x26"),g+=encodeURIComponent(a)+"\x3d"+encodeURIComponent(b));a=this.action;a=0<a.indexOf("?")?a+"\x26"+g:a+"?"+g;this.$root.setUrl(a,h)},resetForm:function(){this.fields=Object.assign({},this.fieldsInitial)},clearForm:function(){this.fields=
{}}},mounted:function(){var a=$(this.$el);this.focusField&&0<this.focusField.length&&a.find("[name\x3d"+this.focusField+"]").addClass("default-focus").focus()}});
...
!0;$.ajax({type:"POST",url:this.valueUrl,data:a,dataType:"text",headers:{Accept:"text/plain"},error:function(c,d,e){b.loading=!1;moqui.handleAjaxError(c,d,e)},success:function(c){b.loading=!1;var d="",e="";try{var f=JSON.parse(c);$.isArray(f)&&f.length?f=f[0]:$.isPlainObject(f)&&f.hasOwnProperty("options")&&f.options.length&&(f=f.options[0]);f.hasOwnProperty("label")&&(d=f.label);f.hasOwnProperty("value")&&(e=f.value)}catch(g){}d&&d.length||(d=c);e&&e.length||(e=c);moqui.isNumber(e)&&(e=e.toString());
b.$emit("input",e);b.fields&&b.fields.length&&b.name&&b.name.length&&(b.fields[b.name+"_display"]=d);b.curDisplay=d}})}else console.warn("In m-display for "+this.name+"  tried to populateFromUrl but not hasAllParms and not dependsOptional"),this.$emit("input",null),this.curDisplay=null;else console.warn("In m-display for "+this.name+" tried to populateFromUrl but no valueUrl")}},computed:{displayValue:function(){return this.curDisplay&&this.curDisplay.length?this.curDisplay:this.value}},mounted:function(){if(this.valueUrl&&
...
$.each(a,function(g,h){g=h[f];var n=h[e];d.push(Object.assign(h,{value:g||n,label:n||g}))});return d},serverData:function(a){var b=!0,c=this.dependsOn,d=this.optionsParameters,e={moquiSessionToken:this.$root.moquiSessionToken};for(g in d)d.hasOwnProperty(g)&&(e[g]=d[g]);for(var f in c)if(c.hasOwnProperty(f)){if(this.fields)d=this.fields[c[f]];else{var g=$("#"+c[f]);(d=g.val())||(d=g.find("select").val())}d?e[f]=d:b=!1}a?(e.term=a.term||"",e.pageIndex=(a.page||1)-1):this.serverSearch&&(e.term="",e.pageIndex=
...
moqui.webrootVue=new Vue({el:"#apps-root",data:{basePath:"",linkBasePath:"",currentPathList:[],extraPathList:[],currentParameters:{},bodyParameters:null,activeSubscreens:[],navMenuList:[],navHistoryList:[],navPlugins:[],accountPlugins:[],notifyHistoryList:[],lastNavTime:Date.now(),loading:0,currentLoadRequest:null,activeContainers:{},urlListeners:[],moquiSessionToken:"",appHost:"",appRootPath:"",userId:"",locale:"en",notificationClient:null,qzVue:null,leftOpen:!1,moqui},methods:{setUrl:function(a,
b,c){this.currentLoadRequest&&(console.log("Aborting current page load currentLinkUrl "+this.currentLinkUrl+" url "+a),this.currentLoadRequest.abort(),this.currentLoadRequest=null,this.loading=0);this.bodyParameters=b;a=this.getLinkPath(a);if(this.currentLinkUrl===a&&a!==this.linkBasePath)this.reloadSubscreens(),c&&this.callOnComplete(c,this.currentPath);else{var d=this.currentPath;b=moqui.parseHref(a);this.extraPathList=[];this.currentSearch=b.search;this.currentPath=b.path;b=this.currentSearch;
if((b=this.currentPath+(0<b.length?"?"+b:""))&&0!==b.length){console.info("Current URL changing to "+b);this.lastNavTime=Date.now();this.activeContainers={};var e=this;b=this.appRootPath&&this.appRootPath.length&&0===b.indexOf(this.appRootPath)?this.appRootPath+"/menuData"+b.slice(this.appRootPath.length):"/menuData"+b;$.ajax({type:"GET",url:b,dataType:"text",error:moqui.handleAjaxError,success:function(f){var g=null;try{g=JSON.parse(f)}catch(h){console.info("Error parson menu list JSON: "+h)}g&&
moqui.isArray(g)&&(e.navMenuList=g,c&&e.callOnComplete(c,d))}});window.history.pushState(null,this.ScreenTitle,a);this.urlListeners.forEach(function(f){f(a,this)},this);document.documentElement.scrollTop=0;document.body.scrollTop=0}}},callOnComplete:function(a,b){if(a){var c=this.getRoute();b&&(c.redirectedFrom=b);a(c)}},getRoute:function(){return{name:this.currentPathList[this.currentPathList.length-1],meta:{},path:this.currentPath,hash:"",query:this.currentParameters,params:this.bodyParameters||
{},fullPath:this.currentLinkUrl,matched:[]}},setParameters:function(a){if(a){this.$root.currentParameters=$.extend({},this.$root.currentParameters,a);a=this.currentLinkUrl;var b=this.navHistoryList[0];b.pathWithParams=a;window.history.pushState(null,b.title||"",a)}this.$root.reloadSubscreens()},addSubscreen:function(a){a.pathIndex=this.activeSubscreens.length;a.loadActive();this.activeSubscreens.push(a)},reloadSubscreens:function(){var a=this.currentPathList,b=this.activeSubscreens;console.info("reloadSubscreens currentPathList "+
...
b?b.reload():console.error("Container with ID "+a+" not found, not reloading")},loadContainer:function(a,b){var c=this.activeContainers[a];c?c.load(b):console.error("Container with ID "+a+" not found, not loading url "+b)},addNavPlugin:function(a){var b=this;moqui.loadComponent(this.appRootPath+a,function(c){b.navPlugins.push(c)})},addNavPluginsWait:function(a,b){if(a&&a.length>b){this.addNavPlugin(a[b]);var c=this;a.length>b+1&&setTimeout(function(){c.addNavPluginsWait(a,b+1)},500)}},addAccountPlugin:function(a){var b=
this;moqui.loadComponent(this.appRootPath+a,function(c){b.accountPlugins.push(c)})},addAccountPluginsWait:function(a,b){if(a&&a.length>b){this.addAccountPlugin(a[b]);var c=this;a.length>b+1&&setTimeout(function(){c.addAccountPluginsWait(a,b+1)},500)}},addUrlListener:function(a){0<=this.urlListeners.indexOf(a)||this.urlListeners.push(a)},addNotify:function(a,b){var c=this.notifyHistoryList.slice(0),d=new Date,e=d.getHours();10>e&&(e="0"+e);d=d.getMinutes();10>d&&(d="0"+d);for(c.unshift({message:a,
type:b,time:e+":"+d});25<c.length;)c.pop();this.notifyHistoryList=c},switchDarkLight:function(){this.$q.dark.toggle();$.ajax({type:"POST",url:this.appRootPath+"/apps/setPreference",error:moqui.handleAjaxError,data:{moquiSessionToken:this.moquiSessionToken,preferenceKey:"QUASAR_DARK",preferenceValue:this.$q.dark.isActive?"true":"false"}})},toggleLeftOpen:function(){this.leftOpen=!this.leftOpen;$.ajax({type:"POST",url:this.appRootPath+"/apps/setPreference",error:moqui.handleAjaxError,data:{moquiSessionToken:this.moquiSessionToken,
preferenceKey:"QUASAR_LEFT_OPEN",preferenceValue:this.leftOpen?"true":"false"}})},stopProp:function(a){a.stopPropagation()},getNavHref:function(a){a||(a=this.navMenuList.length-1);a=this.navMenuList[a];if(a.extraPathList&&a.extraPathList.length){var b=a.path+"/"+a.extraPathList.join("/"),c=a.pathWithParams.indexOf("?");0<c&&(b+=a.pathWithParams.slice(c));return b}return a.pathWithParams||a.path},getLinkPath:function(a){moqui.isPlainObject(a)&&(a=moqui.makeHref(a));this.appRootPath&&this.appRootPath.length&&
0!==a.indexOf(this.appRootPath)&&(a=this.appRootPath+a);a=a.split("/");var b=this.appRootPath.split("/").length;a[b]=this.linkBasePath.split("/").slice(-1);return a=a.join("/")},getQuasarColor:function(a){return moqui.getQuasarColor(a)}},watch:{navMenuList:function(a){if(0<a.length){var b=a[a.length-1];a=1<a.length?a[a.length-2]:null;b.extraPathList&&(this.extraPathList=b.extraPathList);var c=this.basePathSize,d=b.path.split("/").slice(c+1);console.info("nav updated fullPath "+JSON.stringify(d)+" currentPathList "+
JSON.stringify(this.currentPathList)+" cur.path "+b.path+" basePathSize "+c);this.currentPathList=d;this.reloadSubscreens();a=(a?a.title+" - ":"")+b.title;c=b.pathWithParams;var e=c.indexOf("?");if(0<e){d=["pageIndex","orderBySelect","orderByField","moquiSessionToken"];var f=c.substring(e+1).split("\x26");c=c.substring(0,e);e=0;for(var g="",h=0;h<f.length;h++){var n=f[h];c=-1===c.indexOf("?")?c+"?":c+"\x26";c+=n;if(!(3<e)){var m=n.indexOf("\x3d");if(0<m){var l=n.substring(0,m);n=n.substring(m+1);
0<l.indexOf("_op")||0<l.indexOf("_not")||0<l.indexOf("_ic")||0<=d.indexOf(l)||l===n||(0<g.length&&(g+=", "),g+=decodeURIComponent(n),e++)}}}0<g.length&&(70<g.length&&(g=g.substring(0,70)+"..."),a=a+" ("+g+")")}d=this.navHistoryList;for(f=0;f<d.length;)d[f].pathWithParams===c?d.splice(f,1):f++;for(d.unshift({title:a,pathWithParams:c,image:b.image,imageType:b.imageType});25<d.length;)d.pop();document.title=a}},currentPathList:function(a){(a=a[a.length-1])&&$(this.$el).removeClass().addClass(a)}},computed:{currentPath:{get:function(){var a=
...
moqui.localeMap[this.locale]&&(this.locale=moqui.localeMap[this.locale]);this.leftOpen="true"===$("#confLeftOpen").val();var a=$("#confDarkMode").val();this.$q.dark.set("true"===a);this.notificationClient=new moqui.NotificationClient(("https:"===location.protocol?"wss://":"ws://")+this.appHost+this.appRootPath+"/notws");var b=[];$(".confNavPluginUrl").each(function(d,e){b.push($(e).val())});this.addNavPluginsWait(b,0);var c=[];$(".confAccountPluginUrl").each(function(d,e){c.push($(e).val())});this.addAccountPluginsWait(c,
